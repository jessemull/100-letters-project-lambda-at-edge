...
PR workflow uses Node **24** (`actions/setup-node`). Merge/deploy pin the same version. CI runs lint, **typecheck**, Jest (≥80% via `jest.config.js`), and build — no duplicate HTML coverage grep.
